...
class BIMPM(nn.Module):
    def __init__(self, args, data):
        ...
        if self.args.conv == True:
            self.pred_fc1 = nn.Linear(self.args.hidden_size * 4 + self.args.n_fm * 3, self.args.hidden_size * 2)
        else:
        ...
        if self.args.conv == True:
            x = torch.cat(
                [agg_p_last.permute(1, 0, 2).contiguous().view(-1, self.args.hidden_size * 2),
                 conv_p.view(-1, self.args.n_fm),
                 conv_infer.view(-1, self.args.n_fm),
                 conv_h.view(-1, self.args.n_fm),
