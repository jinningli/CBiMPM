...
class CBIMPM(nn.Module):
    def __init__(self, args, data):
        super(CBIMPM, self).__init__()

        self.args = args
        self.d = self.args.word_dim + int(self.args.use_char_emb) * self.args.char_hidden_size
        self.l = self.args.num_perspective

        self.conv = nn.Conv2d(1, self.args.n_fm, (self.args.kernel_size, self.args.hidden_size * 2))
        ...
        if self.args.conv == True:
            self.pred_fc1 = nn.Linear(self.args.hidden_size * 6 + self.args.n_fm * 3, self.args.hidden_size * 3)
        else:
            self.pred_fc1 = nn.Linear(self.args.hidden_size * 6, self.args.hidden_size * 3)

        self.pred_fc2 = nn.Linear(self.args.hidden_size * 3, self.args.class_size)
    ...
    def forward(self, **kwargs):
        ...
        def ESD(v1, v2):
            # (batch, channels)
            eu_norm = (v1 - v2).norm(p=2, dim=1, keepdim=True) #(batch, 1)
            return eu_norm + F.sigmoid(torch.mm(v1.unsqueeze(1), v2.unsqueeze(2)))
            # (batch, 1)
        ...
        # ----- Context Representation Layer -----
        # (batch, seq_len, hidden_size * 2) (2, batch, hidden_size)
        con_p, con_p_out = self.context_LSTM(p)
        con_h, con_h_out = self.context_LSTM(h)

        con_p = self.dropout(con_p)
        con_h = self.dropout(con_h)
        con_p_out = self.dropout(con_p_out[0])
        con_h_out = self.dropout(con_h_out[0])
        ...
        if self.args.conv == True:
            ...
            conv_p = torch.cat((con_p_fw, con_p_bw), 2)
            conv_h = torch.cat((con_h_fw, con_h_bw), 2)
            # (batch, channels, seq_len)
            conv_p = nn.functional.relu(self.conv(conv_p.unsqueeze(1)).squeeze(3))
            conv_h = nn.functional.relu(self.conv(conv_h.unsqueeze(1)).squeeze(3))

            conv_p = self.dropout(conv_p)
            conv_h = self.dropout(conv_h)

            # (batch, channels)
            conv_p = nn.functional.max_pool1d(conv_p, conv_p.size(2))
            conv_h = nn.functional.max_pool1d(conv_h, conv_h.size(2))
            conv_infer = conv_p * conv_h

        con_p_out = con_p_out.permute(1, 0, 2) # (batch, 2, hidden_size)
        con_h_out = con_h_out.permute(1, 0, 2) # (batch, 2, hidden_size)

        con_out_infer = con_p_out * con_h_out# (batch, 2, hidden_size)

        # 2 * (2, batch, hidden_size) -> 2 * (batch, hidden_size * 2) -> (batch, hidden_size * 4)
        if self.args.conv == True:
            x = torch.cat(
                [con_p_out.contiguous().view(-1, self.args.hidden_size * 2),

                 conv_p.contiguous().view(-1, self.args.n_fm),

                 conv_infer.contiguous().view(-1, self.args.n_fm),
                 con_out_infer.contiguous().view(-1, self.args.hidden_size * 2),
                 # ESD(conv_h, conv_p).contiguous().view(-1, 1),

                 conv_h.contiguous().view(-1, self.args.n_fm),

                 con_h_out.contiguous().view(-1, self.args.hidden_size * 2)], dim=1)
        else:
            x = torch.cat(
                [con_p_out.contiguous().view(-1, self.args.hidden_size * 2),
