...
class CONV(nn.Module):
    def __init__(self, args, data):
        ...
        if self.args.conv == True:
            self.pred_fc1 = nn.Linear(self.args.n_fm * 2 + 1, self.args.n_fm)
        else:
            pass

        self.pred_fc2 = nn.Linear(self.args.n_fm, self.args.class_size)
    ...
    def forward(self, **kwargs):
        ...
        def ESD(v1, v2):
            ...
            dt = torch.bmm(v1.view(-1, 1, self.args.n_fm), v2.view(-1, self.args.n_fm, 1))
            smd = F.sigmoid(dt)
            return eu_norm * smd
            # (batch, 1)
        ...
        if self.args.conv == True:
            x = torch.cat(
                [
                    ...
                    conv_h.contiguous().view(-1, self.args.n_fm)], dim=1)
        else:
            pass
